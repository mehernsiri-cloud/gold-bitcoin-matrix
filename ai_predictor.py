...
# ------------------------------
# Load historical data with dynamic macro indicators
# ------------------------------
def load_data(asset_name, actual_col):
    """Load historical asset data including macro indicators from weight.yaml"""
    ...
    # Load macro indicators from YAML
    if os.path.exists(WEIGHT_FILE):
        with open(WEIGHT_FILE, "r") as f:
            weights = yaml.safe_load(f)
        indicators = ["inflation", "usd_strength", "energy_prices", "tail_risk_event"]
        for ind in indicators:
            df_asset[ind] = weights.get(asset_name.lower(), {}).get(ind, 0.0)
    else:
        # fallback to zeros
        for ind in ["inflation", "usd_strength", "energy_prices", "tail_risk_event"]:
            df_asset[ind] = 0.0
...
def create_features(df, window=3):
    """Generate ML features: past actuals + predicted prices + macro indicators"""
    X, y = [], []
    n = len(df)
